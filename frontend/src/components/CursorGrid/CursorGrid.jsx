...
const CursorGrid = ({
  ...
  pulseSpeed = 600,
  trackWindow = false,
  // Explicit opt-out (e.g. a user's "reduce motion" preference). The
  // component also auto-disables itself for prefers-reduced-motion and
  // coarse/touch pointers regardless of this prop.
  disabled = false,
  className = ''
}) => {
  ...
  // Kept in a layout effect (runs synchronously after DOM mutations, before
  // paint) rather than mutated directly in the render body — mutating a
  // ref during render is unsafe under React's stricter render-purity rules
  // (and is what the React Compiler's lint rule flags). Every function
  // inside the main effect below reads propsRef.current fresh on each
  // call, so this update is picked up without needing that effect to
  ...
  useEffect(() => {
    ...
    // Combines the explicit `disabled` prop with OS-level reduced-motion
    // preference and touch/coarse-pointer devices, where a cursor-chasing
    // effect is either unwanted or meaningless (no persistent hover point).
    const isActive = () =>
    ...
    const wake = () => {
      if (running) return;
      // Also gates the animation loop itself, not just input handling — so
      // a disabled/reduced-motion/coarse-pointer state never spins rAF.
      if (!isActive()) return;
      running = true;
    ...
    const onPointerDown = e => {
      if (!isActive() || !propsRef.current.clickPulse) return;
      const [x, y] = toLocal(e);
      pulses.push({ x, y, t0: performance.now() });
      wake();
    };

    // Pause the animation loop entirely while the tab isn't visible —
    // there's no point burning CPU/battery animating something no one
    // can see. Resuming is automatic on the next pointer move; wake()
    // here just picks back up immediately if something was still fading.
    ...
    // For a page-wide background the container itself is pointer-events:none
    // (so clicks reach the real UI beneath), which means it never receives
    // pointer events directly — track the window instead so it still
    // reacts to the cursor everywhere on screen. Falls back to the
    // container-scoped behavior for a bounded, non-fullscreen usage.
    const eventTarget = propsRef.current.trackWindow ? window : container;
